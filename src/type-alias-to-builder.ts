...
import { getTypeDataTypes as getTypeAliasDataTypes } from './utils/type-alias-data-types-util'
...
export const execute = (workspaceRoot: string, window: IWindow) => {
  ...
  const typeName = getTypeAliasName(text, window)
  if (!typeName) return
  const properties = getTypeAliasProperties(text, window)
  const dataTypes = getTypeAliasDataTypes(text, window)
